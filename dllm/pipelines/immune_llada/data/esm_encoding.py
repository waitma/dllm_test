"""Shared ESM-family tokenization adapters.

Use ``Esm2SequenceTokenizer`` for the built-in vocabulary or
``HuggingFaceEsmTokenizerAdapter.from_pretrained(path)`` for a local tokenizer
snapshot; no network or offline token cache is required.
"""
