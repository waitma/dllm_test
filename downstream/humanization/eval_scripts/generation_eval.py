...
# Delay import to avoid apex/pyramid conflict at startup
# from immunomatch_score import main as compute_immunomatch_score
# from humatch_score import main as compute_humatch_score
...
def main(generate_results_file: str, gen_col_name: str, ref_col_name: str = None, 
    ...
    # Lazy import to avoid apex/pyramid conflict
    from immunomatch_score import main as compute_immunomatch_score
    ...
    # Evaluate Humatch CNN_P scores (humanness)
    # print("\n" + "="*50)
    # print("Computing Humatch CNN_P scores...")
    # print("="*50)
    
    # Lazy import to avoid apex/pyramid conflict
    from humatch_score import main as compute_humatch_score
    
    # # Initialize ref humatch scores and comparison
    # ref_humatch_mean = None
    # gen_humatch_better_ratio = None
    
    # # If reference sequences are provided, compute their Humatch scores
    # if ref_col_name:
    #     ref_humatch_scores = compute_humatch_score(
    #         df_dir=generate_results_file,
    #         hseq_col=heavy_col_name,
    #         lseq_col=ref_col_name,
    #         species_col='species'
    #     )
    #     ref_humatch_mean = np.mean(ref_humatch_scores)
        
    #     # Calculate the ratio where gen_humatch_score > ref_humatch_score
    #     gen_humatch_better_count = sum(1 for gen_score, ref_score in zip(gen_humatch_scores, ref_humatch_scores)
    #                                    if gen_score > ref_score)
    #     gen_humatch_better_ratio = gen_humatch_better_count / len(gen_humatch_scores)
    
    # results['gen_humatch_mean'] = gen_humatch_mean
    # if ref_humatch_mean is not None:
    #     results['ref_humatch_mean'] = ref_humatch_mean
    #     results['gen_humatch_better_ratio'] = gen_humatch_better_ratio
    ...
    if ref_col_name:
        ...
        if 'gen_better_ratio' in results:
            print(f"Gen > Ref Immunomatch Ratio: {results['gen_better_ratio']:.4f}")
        if 'ref_humatch_mean' in results:
            print(f"Ref Humatch CNN_P Mean: {results['ref_humatch_mean']:.4f}")
        if 'gen_humatch_better_ratio' in results:
            print(f"Gen > Ref Humatch Ratio: {results['gen_humatch_better_ratio']:.4f}")
    if ref_col_name and detailed_comparison:
