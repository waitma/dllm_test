...
- `ESMCEncoderBioDiffusionModel` is the encoder-conditioned target.
- The no-encoder antibody model uses the migrated Ophiuchus-Ab ESM2 transformer with chain-aware multimer attention.
- The encoder model uses ESMC as a tunable condition encoder by default and projects encoder states into the diffusion decoder; freezing ESMC should be treated only as a bootstrap or ablation setting.
...
- Model layer v2: the encoder-conditioned path should add ESMC/ESM2 features as trainable conditioning features, but keep the objective diffusion-only. Fixed context chains, such as antigen in antibody-antigen generation, remain clean and do not receive direct reconstruction loss.
...
- Training should separate biological examples from training views. The stored example contains the full clean chains plus regions/metadata; the collator/view sampler chooses a task view and creates token-level masks for that step.
- For the Qwen3-VL-style BioSeq path, training-time data loading lives under `/vepfs-mlp2/c20250601/251105016/project/dllm_test/dllm/pipelines/qwen3_vl_arch/data`. This path should read source-specific files directly, emit canonical `BioSeqRecord` objects, sample training views, and collate ESM-family token ids plus per-chain encoder tensors. Full offline JSONL conversion is optional, not a prerequisite for training.
- The Qwen3-VL-style BioSeq model layer now starts at `/vepfs-mlp2/c20250601/251105016/project/dllm_test/dllm/pipelines/qwen3_vl_arch/modeling_bioseq.py`. It exposes `BioSeqNoEncoderDiffusionModel` for decoder-only training and `BioSeqEncoderDiffusionModel` for ESMC/ESM-conditioned training.
- The first Qwen3-VL-style decoder is a dense bidirectional diffusion transformer with ESM-family token embeddings, chain-local residue position embeddings, outer chain-index embeddings, chain-role embeddings, task-type embeddings, timestep embeddings, RMSNorm, and SwiGLU blocks. It is intentionally separate from the legacy `/vepfs-mlp2/c20250601/251105016/project/dllm_test/dllm/pipelines/bioseq` lightweight backend.
...
- `BioSeqEncoderDiffusionModel.compute_loss` uses the same diffusion objective and additionally applies the decoder corruption mask back onto `encoder_input_ids` before running the encoder. This prevents clean target residues from leaking through ESMC/ESM features while keeping fixed context chains such as antigen, peptide, and MHC clean.
...
- Training should use `/vepfs-mlp2/c20250601/251105016/project/dllm_test/dllm/pipelines/qwen3_vl_arch/data/mixture.py::TaskHomogeneousBatchDataset` before collation. Each physical microbatch should contain one task group and one shared generation view sampled by `BioSeqQwenDataCollator`. Sample de-duplication is expected from data processing; batch-level de-duplication is an optional defensive setting, not the default training path.
- Multi-node/multi-GPU training for the Qwen3-VL-style BioSeq path should use `/vepfs-mlp2/c20250601/251105016/project/dllm_test/examples/bioseq/train_qwen3_vl_bioseq_ddp.py` launched by `torchrun`. It supports ordinary single-process smoke runs, single-node multi-GPU, and multi-node multi-GPU through `RANK`, `WORLD_SIZE`, and `LOCAL_RANK`.
- Qwen3-VL-style streaming data is DDP-sharded inside `/vepfs-mlp2/c20250601/251105016/project/dllm_test/dllm/pipelines/qwen3_vl_arch/data/mixture.py`: source iterators now use `global_shard_index = rank * num_workers + worker_id` and `num_shards = world_size * num_workers`, so DDP ranks do not read identical iterable records by default.
...
- Initial view sampling should stay simple: `BioSeqViewSampler(full_denoise_probability=0.5)` samples `full_denoise` with probability 0.5, then samples uniformly from compatible condition views with the remaining probability 0.5. If no condition view is compatible, the batch falls back to `full_denoise`.
- To reduce multi-task gradient jitter, optimizer updates should use gradient accumulation across several task-homogeneous microbatches, or a later weighted scheduler that controls task order. Do not rely on a randomly mixed record stream with ordinary `DataLoader(batch_size=N)` for foundation training.
...
- In the Qwen3-VL-style loader, `full_denoise` means denoising all eligible target chains, not every chain unconditionally. Explicit `metadata["targets"]` is honored when present, but antigen, peptide, MHC, and HLA-like chains are fixed context by default and do not receive diffusion loss.
...
- Encoder-conditioned training is still diffusion training: ESMC/ESM2 features condition target denoising, but the encoder is updated through the target diffusion loss rather than through a separate antigen reconstruction loss.
- The encoder-conditioned path defaults to trainable encoder parameters. `freeze_encoder=True` is available only for ablations or bootstrap checks; the main foundation-model setting should fine-tune the ESMC/ESM encoder and connector with the diffusion loss.
